...
function App() {
  ...
  useEffect(() => {
    if (bet) {
      const newRandomNumbers: SetStateAction<number[]> = [];
      while (newRandomNumbers.length < mines) {
          ...
          <div className="bg-[#2a2e35] rounded-lg">
            <div className="rounded-lg p-4 space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-gray-400 flex items-center align-center gap-1">
                  Amount
              ...
              <div className="flex items-center justify-between gap-2 bg-[#1a1d24] rounded-lg px-2">
                <div className="flex items-center">
                  <img src={INR} alt="Currency" className="w-5 h-5" />
                  <input
                    type="number"
                    value={betAmount}
              ...
              <div className="grid grid-cols-4 gap-2">
                {[10, 100, "1.0K", "10.0K"].map((value) => (
                  <button
                    key={value}
                    className="bg-[#1a1d24] py-2 rounded text-sm"
                    onClick={() => setBetAmount(value === "1.0K" ? 1000 : value === "10.0K" ? 10000 : Number(value))}
                    disabled
                  >
                    {value}
                  </button>
              ...
              {!bet && (
                <button
                  className="w-full bg-gradient-to-r from-green-500 to-lime-500 hover:bg-green-600 text-white py-3 rounded-lg font-semibold"
              ...
              {clickBock.length >= 1 && !gameOver && (
                <>
                  <h3 className="text-gray-400 flex items-center align-center gap-1">Gems</h3>
                  <input
                    type="number"
                    value={gems + 1 - Number(clickBock.length)}
                    onChange={(e) => setGems(Number(e.target.value))}
                    className="bg-[#1a1d24] p-2 rounded-lg w-full"
                  />
                </>
              )}
              ...
              {bet && (
                <button
                  className="w-full bg-neutral-800 text-white py-3 rounded-lg font-semibold flex items-center justify-center"
                ...
                >
                  ...
                  >
                    <path
                      fill="currentColor"
                      d="M8.427 6.68c-1.592.306-1.85.554-2.168 2.081-.318-1.527-.576-1.775-2.167-2.08 1.591-.306 1.85-.554 2.167-2.081.318 1.527.576 1.775 2.168 2.08m7.007-.221c-1.352-.864-1.715-.864-3.065 0 .9-1.298.9-1.646 0-2.943 1.352.864 1.714.864 3.065 0-.9 1.297-.9 1.646 0 2.943M8.38 15.488c-1.353-.864-1.715-.864-3.066 0 .9-1.298.9-1.646 0-2.942 1.353.864 1.715.864 3.066 0-.9 1.296-.9 1.646 0 2.942m11-4.676c-1.201-.77-1.528-.77-2.728 0 .801-1.154.801-1.467 0-2.62 1.202.77 1.528.77 2.728 0-.8 1.154-.8 1.466 0 2.62M10.469 9.194l-.416.4c-.459.44-.459 1.155 0 1.595l9.145 8.78c.459.44 1.203.44 1.662 0l.416-.4c.46-.44.46-1.155 0-1.596l-9.145-8.779a1.21 1.21 0 0 0-1.662 0"
                    ></path>
                    <path
                      fill="currentColor"
                      d="m10.053 9.593.416-.399a1.2 1.2 0 0 1 .83-.33c.313 0 .612.118.832.33l1.247 1.197-2.078 1.995-1.247-1.197a1.1 1.1 0 0 1-.344-.798c0-.3.124-.586.344-.798"
                    ></path>
                  </svg>
